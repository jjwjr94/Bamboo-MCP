...
-   **Superior Developer Experience**:
    ...
    -   **Schema Generation**: `scripts/generateSchemas.ts` generates Zod schemas directly from the Meta SDK, ensuring our server stays in sync with API changes.
    -   **CSP Hash Automation**: `scripts/generateCSPHashes.ts` auto-discovers inline scripts and generates SHA-256 hashes for zero-touch Content Security Policy management.
...
| Script                  | Description                                                                                             |
| ----------------------- | ------------------------------------------------------------------------------------------------------- |
| `prebuild:schemas`      | Generates Zod schemas from Meta SDK using TypeScript (`tsx scripts/generateSchemas.ts`).               |
| `prebuild:csp`          | Auto-discovers scripts and generates CSP hashes using TypeScript (`tsx scripts/generateCSPHashes.ts`). |
| `prebuild`              | Runs both `prebuild:schemas` and `prebuild:csp` to generate all required build artifacts.              |
...
-   `src/generated`: Contains auto-generated files:
    -   `schemas.ts`: Zod schemas generated by `scripts/generateSchemas.ts`
    -   `cspHashes.ts`: CSP hash constants generated by `scripts/generateCSPHashes.ts`
