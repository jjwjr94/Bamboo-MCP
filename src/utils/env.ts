...
const envSchema = z
  ...
  .superRefine((data, ctx) => {
    // Conditionally require BASE_URL for non-test environments
    if (data.NODE_ENV !== 'test' && !data.BASE_URL) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'BASE_URL is required for development and production environments.',
        path: ['BASE_URL'],
      });
    }
  });
 ...
 * @throws InitializationError if BASE_URL is not available when required
 */
